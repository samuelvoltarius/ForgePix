...
def sample(batch, size, device, generator):
    ...
    for _ in range(3):
        ...
        clean += torch.exp(-((xx - cx).square() + (yy - cy).square()) / (2 * sigma.square())) * rand(batch, 3, 1, 1) * 0.06
...
def main():
    ...
    with (args.output / "metrics.jsonl").open("w") as log:
        for step in range(1, args.steps + 1):
            noisy, clean = sample(args.batch, args.size, device, generator)
        ...
        model.eval()
        with torch.no_grad():
            noisy, clean = sample(8, args.size, device, validation_generator)
            result = model(noisy)
            validation = dict(input_mse=(noisy-clean).square().mean().item(),
