...
Changes after the audited commit require the corresponding row to be rechecked.
...
| Mono/OSC sensor input | Partial: `astro.read_calibrated` calibrates raw CFA before debayer and honors Bayer offsets; unlabeled mono remains mono. | Numerical FITS tests cover order and Bayer behavior. General FITS reader expects primary-HDU images; SCI/WHT/DQ archive cubes need explicit handling. Debayer still uses uint16 conversion. | RC / Next |
| Dark, flat, bias/dark-flat calibration | Partial: raw masters, finite/shape checks, FITS-first master selection, unsigned subtraction fixed. | `test_astro_release` covers malformed inputs. M27 run had no matching calibration frames, so real calibrated end-to-end evidence is still missing. | RC |
| Calibration library / automatic frame compatibility | Partial: folder discovery and dimensions exist. | Matching camera, gain, offset, readout mode, binning, temperature, exposure and flat filter/session is not comprehensively enforced. Equipment presets cannot make incompatible masters valid. | RC |
| Cosmetic correction and banding | Native: `astro.cosmetic_correct`, `fix_banding`. | `test_astro_gaps` has numerical banding/gradient-preservation fixtures; more real hot-pixel/amp-glow validation needed. Neither feature substitutes for suitable darks/flats. | RC validation |
...
| Translation, rotation and triangle star matching | Native/partial: `astro.register_and_cache`, triangle fallback, affine matching and cluster rescue. | `test_astro_gaps` covers rotation/mirroring; M27 aligned 31/31 selected frames. Audited cache used reflected borders and uint16 TIFF, requiring precision/coverage fixes. | RC |
...
| Sigma, Winsor, linear-fit, mean/median integration | Native/partial: `astro.stack` has iterative rejection and optional inverse-background-variance weights. | Synthetic outlier/SNR tests exist. Audited integration lacks registration-coverage and rejection maps; invalid border samples must not influence sums or statistics. | RC |
...
| Drizzle and CFA/Bayer drizzle | Partial: separate point/drop-like reconstruction exists; ordinary `drizzle` scaling uses interpolation. | Audited `drizzle_stack` already debayers input, resizes incompatible shapes and fills uncovered samples through uint8 inpainting. No native CFA drizzle, validated flux/coverage kernel or real dither benchmark. Do not label this full Siril Drizzle parity. | RC guard / Next implementation |
| Live stacking / restart | Partial: `livestack.LiveStack` saves accumulators and reference and rejects invalid frames/checkpoints. | `test_livestack` and `test_live_regressions` cover state/color regressions. Configuration/master identity on resume and coverage/rotation consistency remain open at audit commit. | RC |
...
| Catalog-color PCC | Partial approximation: local/online Gaia paths obtain positions, then `_fit_channel_gains` neutralizes median image patches at those positions. | The fit does **not** use queried `bp_rp` or catalog magnitudes to predict/filter expected colors. It is catalog-selected white balance, not verified catalog-color calibration. Name it accordingly until implemented. | RC naming / Next implementation |
...
| Measured narrowband channel combination | Partial: H-alpha/OIII extraction and display palettes; filter guards prevent applying H-alpha assumptions to known SII/OIII data. | User SV220 SII/OIII 7 nm has a distinct profile. Dedicated SII/OIII workflow and aligned independent Ha/OIII/SII/LRGB source combinations are incomplete at audit commit. Synthetic SHO cannot recover unmeasured H-alpha or sulfur. | Next, high |
...
| Native star separation/recombination | Partial engine: `astro.remove_stars` uses morphology and uint8 Telea inpainting; `stretch_starless` uses it. Full `starless.run` requires StarNet, optionally GraXpert/Cosmic Clarity, and caches 16-bit layers. | Synthetic star-energy tests do not establish reliable separation of dense fields, halos, galaxies or faint nebulae. Need float layers, exact neutral recombination and real validation. | Next, high |
...
|---|---|---|
| GraXpert automatic background / denoising | Research for own models; native classical alternatives exist. `training/train_synthetic.py` has separate tasks. `graxpert_engine` is an external bridge. | Representative gradients and independent real noise pairs; preserve extended emission and stellar flux. Next, no default promotion without evidence. |
| BlurXTerminator-like local aberration/deblur | Research: synthetic isotropic-blur training plus classical tiled RL. `cosmicclarity_engine` and Siril Python bridges are external. | Train and validate field-varying PSFs, aberrations, saturated stars and faint structures. No measured quality parity. Later. |
| StarXTerminator-like learned separation | Research: synthetic starless target task. | Dense fields, galaxies, diffraction spikes and halos need trusted targets plus false-removal tests. Keep native classical limitations visible. Next / Later. |
| Camera-general ML inference | Missing as a qualified production feature. A small own-weight NAFNet family was trained on Spark; upstream architecture license is retained. | No accepted ONNX package or CPU/GPU/tile-boundary release benchmark. HST scene adaptation worsened validation versus parent and was rejected; more iterations alone do not fix it. |
...
| Reversible project history, reusable processing recipes | Partial/unverified: settings and intermediate exports exist. | No complete native project document with per-step parameters, masks, undo/redo and reproducible re-opened processing graph found. Next, high. |
...
2. Complete the native daily workflow: channel-aware SII/OIII and independent SHO,
   float star layers/recombination, editable masks, reversible history and clear
...
4. Implement native astrometry, actual catalog-color/SPCC calculations, CFA
   drizzle, WCS mosaics and measured calibration libraries as independent features
