...
Rows below were updated for the 2026-09-06 integration, native layer and own-AI work through runtime `441b432`; execution evidence is in RELEASE_WORKLOG.md.
...
| Camera-general ML inference | Local CPU ONNX execution exists with four own mono models and independent colour channels. Background estimates a smooth whole-field additive residual; other tasks use fixed 256px overlapping tiles. Manifests/weights/outputs have integrity checks, retained after verified reimport. | Camera-general quality is still unqualified. Actual larger-image tests detect tile-phase dependence. HST scene bases retain observational noise and do not provide real clean/noisy pairs. Models remain explicit experiments. |
