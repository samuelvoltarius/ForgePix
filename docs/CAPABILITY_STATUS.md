...
| Camera-general ML inference | Local CPU ONNX execution now exists with four own mono models, fixed 256px overlapping tiles and independent colour channels. Manifests/weights/outputs have integrity checks. | Camera-general quality is still unqualified. Actual larger-image tests detect tile-phase dependence. HST scene bases retain observational noise and do not provide real clean/noisy pairs. Models remain explicit experiments. |
