...
Reviewed 2026-09-06 against the native catalogue/recipe additions and evidence in
...
path passed independent checks on all 31 matrices at a60a832; worst median
1.083 px and P95 1.917 px. Older successful-registration counts alone must not
...
| Stack time, reference hints and accepted coverage | Native FITS acquisition summary from actual contributors/reference; matching FITS/Float32-TIFF headers, exposure-midpoint time estimate, separate total exposure and accepted common coverage after rejection/binning. | No inferred historical filter, output gain/saturation/variance or WCS. Time weighting remains an explicit estimate. Tests cover dropped frames, mixed/missing fields, time scales, reference sampling and project/export. See NATIVE_OBSERVATION_METADATA.md. | Next real-data and exact-package acceptance |
...
| Drizzle and CFA/Bayer drizzle | Native square-drop reconstruction: original calibrated CFA samples retained, exact subpixel/affine area overlaps, Float64 sums, Float32 results, per-channel weights/coverage and 1x/2x GUI/CLI path. | Analytic and independent geometry tests plus corrected full 34-FITS M27 run at a60a832: QC kept 31, all matrices independently checked across the meridian flip, 99.999906% common RGB coverage at 1x. This is geometric/coverage acceptance, not recovered-detail proof. No filling, clipping, silent shape changes or sigma rejection. No exposure normalization, WCS distortion/TPS, or CFA cosmetic/banding combination. See NATIVE_DRIZZLE.md. | Next broader scientific validation |
...
| Native plate solving, WCS refinement and annotations | Native hinted local solver: `astrometry` matches triangles with free rotation/parity, TAN/affine WCS and held-out validation; `astrometry_file` preserves FITS values/types/units and companions. Catalogue GUI and bounded native ESA TAP download available. | Full M27 GUI/project/export: 154 fit + 78 independent stars, RMS .427 px; pixel hashes unchanged. Geometry and file/GUI tests. No blind search, distortion refinement, proper-motion propagation or annotation overlay. See NATIVE_ASTROMETRY.md. | Next broader fields, epoch and distortion |
| Catalog-color PCC | Actual colour fitting remains missing. New native diagnostic foundation: separate Gaia/GSPC field with exact IDs, epoch/PM/quality and BVR data; shared RGB apertures, local sky, flux/error/exclusion JSON/CSV through CLI. Existing Auto/Lite remains neutral-population stellar white balance. | Analytic flux, independent noise/epoch/unit/WCS tests and real M27 diagnostics. Unknown filters, saturation, coverage and timing stay explicit; no gains or image writes. GSPC BVR is not camera RGB. Full gated fit, aperture correction, qualified camera data and guided GUI remain open. See NATIVE_PHOTOMETRY.md and NATIVE_COLOR_CALIBRATION_PLAN.md. | Next gated empirical PCC and GUI |
...
| Executable macros / recipes | Native versioned `.fprecipe` files with ordered pinned AI models, strengths, device preference, source snapshot, atomic run journal and cancellation; GUI editing/save/load and public CLI. | Real full M27 GUI recipe, two DirectML models, restart and both project steps verified. First subset covers four own AI tasks; no action recorder, masks, PixelMath adapter, multi-input queue or arbitrary processing graph yet. See RECIPES.md. | Next native process adapters and batch |
...
2. Complete the native daily workflow: channel-aware SII/OIII and independent SHO,
   ...
   previews/errors. Remove scientific labels from approximations. The darktable
   and Siril-script review now prioritizes classical native background and MTF/GHS
   recipe adapters, shared masks, domain-aware previews and then per-session
   multi-night manifests; see DARKTABLE_RESEARCH.md and SIRIL_SCRIPT_RESEARCH.md.
